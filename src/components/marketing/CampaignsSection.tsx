    ...
    <article className="overflow-hidden rounded-[2rem] border border-red-100 bg-white shadow-soft">
      <div className="relative overflow-hidden bg-[#f7f0ea]">
        <img
          ...
          alt={campaign.title}
          className="h-[260px] w-full object-cover object-center sm:h-[300px] lg:h-[340px]"
        />
        <div className="absolute inset-0 bg-[linear-gradient(to_top,rgba(255,255,255,0.94)_0%,rgba(255,255,255,0.68)_24%,rgba(255,255,255,0.08)_62%,rgba(255,255,255,0)_100%)]" />
        <div className="absolute left-5 top-5 inline-flex items-center gap-2 rounded-full bg-white/90 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-brand-red shadow-sm backdrop-blur">
          <Sparkles className="h-3.5 w-3.5" />
          {statusLabel}
        </div>

        <div className="absolute inset-x-0 bottom-0 p-4 sm:p-5 lg:p-6">
          <div className="rounded-[1.75rem] border border-white/70 bg-white/92 p-4 shadow-[0_16px_40px_rgba(15,23,42,0.12)] backdrop-blur-sm sm:p-5">
            ...
            <Link
              ...
              className="mt-4 inline-flex items-center gap-2 rounded-full bg-brand-red px-5 py-2.5 text-sm font-semibold text-white shadow-soft transition hover:bg-brand-dark"
            >
              {actionLabel}
  ...
  return (
    <article className="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="mt-3 flex flex-wrap gap-2 text-[11px] text-slate-500">
        <span className="rounded-full bg-slate-50 px-2.5 py-1">{dateLabel || (locale === "ar" ? "دون تاريخ" : "Date non précisée")}</span>
